...
import sys 
sys.path.append(r"/data5/store1/hxw/MYTRF")
import myleast_squares
#from scipy.optimize import least_squares #只有非线性部分用到了自己的scipy其他全是官方的
...
def L_curve(mat,F,grads_y1,grads_y2,lambda_x_y,lamb_regu,solver,device,cupy_device):
    iter=len(lamb_regu)
    res=np.zeros([1,iter]) #储存每次正则化参数的对应的最小二乘残差
    reg_norm=np.zeros([1,iter]) #储存每次正则化参数对应的正则化参数的范数

    n=mat.shape[1]
    w_store=np.zeros([n,iter]) #每一列为不同的正则化参数对应的权值
    ...
    for i in range(iter):
        ...
        if solver=="linear":
            ...
            w=myleast_squares.least_squares(F_2D.cal_f,x0,J_2D.cal_j,args=(mat_regu,F_regu,grads_y1,grads_y2,lambda_x_y,device,cupy_device),ftol=1e-6,gtol=1e-6,xtol=1e-6,verbose=2,cupy_device=cupy_device)
            w_x=(w.x).reshape(-1,1)
