...
class Cell:
    def __init__(self, x0, y0,z0, size, x,level=0):
        self.x0 = x0
        self.y0 = y0
        self.z0 = z0
        self.x= x
    ...
    def pre_calculate(self, n):
        g=generate_data3.GaussLegendre3D_rec(n,self.x0,self.x0+self.size,self.y0,self.y0+self.size,self.z0,self.z0+self.size,self.x)
    ...
    def refine(self, n):
        ...
        for dx in [0, h]:
            for dy in [0, h]:
                for dz in [0,h]:
                    child = Cell(self.x0 + dx, self.y0 + dy,self.z0+dz, h, self.x,self.level + 1)
...
def collect_all_gauss_points(cells):
    ...
    W = np.vMA-RFM-GITstack(all_weights)
    return P, W

def create_initial_grid(x_min,x_max,y_min,y_max,z_min,z_max,N, n,x):
    cells = []
    h = (x_max-x_min) / N
    for i in range(N):
        for j in range(N):
            for m in range(N):
                cell = Cell(x_min+i * h, y_min+j * h,z_min+m*h, h,x)
