...
class AdaptiveMeshRefinementFix:
    ...
    def _should_refine_multidim(self, metrics, dynamic_threshold_abs, dynamic_threshold_grad, dynamic_threshold_noise):
        indicator_S_fine = metrics["indicator_S_fine"]
        indicator_grad_fine = metrics["indicator_grad_fine"]
        # Keep dynamic_threshold_grad in the signature for legacy compatibility.
        del dynamic_threshold_grad

        cond_s = indicator_S_fine > dynamic_threshold_abs
        cond_grad = indicator_grad_fine > dynamic_threshold_noise
        return cond_s or cond_grad
        ...
        while iter<current_maxiter:
            ...
            for i, cell in enumerate(cells_to_process):
                ...
                for dx in [0, h]:
                    for dy in [0, h]:
                        for dz in [0,h]:
                            child = mesh.Cell(cell.x0 + dx, cell.y0 + dy,cell.z0+dz ,h, cell.x, cell.level + 1)
                ...
                num_p = 8 * (self.nx * self.nx * self.nx)
            ...
            if is_multidim and len(cells_to_process) > 0:
                ...
                dynamic_threshold_noise = threshold_noise
