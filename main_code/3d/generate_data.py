...
class forward_operator_rec():
    ...
    def _matmul_cupy_chunked(self, left, right, batch_number):
        if isinstance(right, torch.Tensor):
            right = right.detach().cpu().numpy()
        right = cp.asarray(np.ascontiguousarray(right))
        if batch_number <= 1:
            result = left @ right
        ...
        col_block = max(1, math.ceil(n_cols / batch_number))
        chunks = []
        for start in range(0, n_cols, col_block):
            end = min(start + col_block, n_cols)
            chunk = left @ right[:, start:end]
            cp.cuda.Stream.null.synchronize()
            chunks.append(chunk)
        result = cp.concatenate(chunks, axis=1)
        del chunks, right
        return result
    ...
    def int_f(self,SS,batch_number,device,cupy_device):
        ...
        if self.mea==1:
            result=self._matmul_cupy_chunked(w_sol, SS, batch_number)
            del w_sol
    ...
    def int_df_x1(self,SS,batch_number,device,cupy_device):
        ...
        if self.mea==1:
            result_y1=self._matmul_cupy_chunked(w_sol_y1, SS, batch_number)
            del w_sol_y1
    ...
    def int_df_x2(self,SS,batch_number,device,cupy_device):
        ...
        if self.mea==1:
            result_y2=self._matmul_cupy_chunked(w_sol_y2, SS, batch_number)
            del w_sol_y2
        if self.mea==0:
            batch_size=int(SS.shape[0]/batch_number)
    ...
    def int_df_x3(self,SS,batch_number,device,cupy_device):
        ...
        if self.mea==1:
            result_y3=self._matmul_cupy_chunked(w_sol_y3, SS, batch_number)
            del w_sol_y3
        if self.mea==0:
            batch_size=int(SS.shape[0]/batch_number)
...
class forward_operator_torus():
    ...
    def _shared_dim_chunks(self, total_n, batch_number, max_chunk_n=50000):
        chunk_count = max(1, batch_number)
        auto_chunks = math.ceil(total_n / max_chunk_n)
        chunk_count = max(chunk_count, auto_chunks)
        chunk_n = math.ceil(total_n / chunk_count)
        for start in range(0, total_n, chunk_n):
            end = min(start + chunk_n, total_n)
            yield start, end

    def _as_cupy_column(self, SS):
        if isinstance(SS, torch.Tensor):
            SS = SS.detach().cpu().numpy()
        return cp.asarray(np.ascontiguousarray(SS))

    def _kernel_phi(self, rr_chunk):
        return cp.exp(1j * self.k * rr_chunk) / (4 * cp.pi * rr_chunk)

    def _kernel_dphi(self, rr_chunk):
        return cp.exp(1j * self.k * rr_chunk) * ((1j * self.k * rr_chunk - 1) / (4 * cp.pi * rr_chunk**2)) / rr_chunk

    def _apply_weighted_kernel(self, rr_cpu, ss, batch_number, extra_cpu=None):
        cp.cuda.Device(self.cupy_device).use()
        ss_cp = self._as_cupy_column(ss)
        result = None
        base_weight = self.J * self.w
        for start, end in self._shared_dim_chunks(rr_cpu.shape[1], batch_number):
            rr_chunk = cp.asarray(rr_cpu[:, start:end])
            kernel_chunk = self._kernel_phi(rr_chunk)
            weight_chunk = base_weight[start:end]
            if extra_cpu is not None:
                extra_chunk = cp.asarray(extra_cpu[:, start:end])
                kernel_chunk = kernel_chunk * extra_chunk
                del extra_chunk
            left_chunk = kernel_chunk * weight_chunk
            partial = left_chunk @ ss_cp[start:end, :]
            result = partial if result is None else result + partial
            del rr_chunk, kernel_chunk, weight_chunk, left_chunk, partial
            cp.cuda.Stream.null.synchronize()
        del ss_cp
        return result

    def _apply_weighted_dkernel(self, rr_cpu, extra_cpu, ss, batch_number):
        cp.cuda.Device(self.cupy_device).use()
        ss_cp = self._as_cupy_column(ss)
        result = None
        base_weight = self.J * self.w
        for start, end in self._shared_dim_chunks(rr_cpu.shape[1], batch_number):
            rr_chunk = cp.asarray(rr_cpu[:, start:end])
            kernel_chunk = self._kernel_dphi(rr_chunk)
            extra_chunk = cp.asarray(extra_cpu[:, start:end])
            weight_chunk = base_weight[start:end]
            left_chunk = kernel_chunk * extra_chunk * weight_chunk
            partial = left_chunk @ ss_cp[start:end, :]
            result = partial if result is None else result + partial
            del rr_chunk, kernel_chunk, extra_chunk, weight_chunk, left_chunk, partial
            cp.cuda.Stream.null.synchronize()
        del ss_cp
        return result
    ...
    def int_f(self,SS,batch_number,device,cupy_device):
        ...
        cp.cuda.Device(self.cupy_device).use()
        if self.mea==1:
            result = self._apply_weighted_kernel(self.RR, SS, batch_number)
        else:
            phi = self._kernel_phi(cp.asarray(self.RR))
            w_sol = (self.J * phi * self.w).get()
            del phi
    ...
    def int_df_x1(self,SS,batch_number,device,cupy_device):
        ...
        cp.cuda.Device(self.cupy_device).use()
        if self.mea==1:
            result_y1 = self._apply_weighted_dkernel(self.R_x1, self.sub_y1, SS, batch_number)
        else:
            dphi_x1 = self._kernel_dphi(cp.asarray(self.R_x1))
            w_sol_y1 = (self.J * dphi_x1 * cp.asarray(self.sub_y1) * self.w).get()
            del dphi_x1
            if isinstance(SS, np.ndarray):
                SS = torch.tensor(SS).to(self.device) 
            batch_size=int(w_sol_y1.shape[0]/batch_number)
    ...
    def int_df_x2(self,SS,batch_number,device,cupy_device):
        ...
        cp.cuda.Device(self.cupy_device).use()
        if self.mea==1:
            result_y2 = self._apply_weighted_dkernel(self.R_x2, self.sub_y2, SS, batch_number)
        else:
            dphi_x2 = self._kernel_dphi(cp.asarray(self.R_x2))
            w_sol_y2 = (self.J * dphi_x2 * cp.asarray(self.sub_y2) * self.w).get()
            del dphi_x2
            if isinstance(SS, np.ndarray):
                SS = torch.tensor(SS).to(self.device) 
            batch_size=int(w_sol_y2.shape[0]/batch_number)
    ...
    def int_df_x3(self,SS,batch_number,device,cupy_device):
        ...
        cp.cuda.Device(self.cupy_device).use()
        if self.mea==1:
            result_y3 = self._apply_weighted_dkernel(self.R_x3, self.sub_y3, SS, batch_number)
        else:
            dphi_x3 = self._kernel_dphi(cp.asarray(self.R_x3))
            w_sol_y3 = (self.J * dphi_x3 * cp.asarray(self.sub_y3) * self.w).get()
            del dphi_x3
            if isinstance(SS, np.ndarray):
                SS = torch.tensor(SS).to(self.device)  
            batch_size=int(w_sol_y3.shape[0]/batch_number)
...
class forward_operator_circle():
    def __init__(self,k,RR,w,r_points,theta_points,sub_y1,sub_y2,sub_y3,mea,device,cupy_device):
        self.mea=mea
        self.k=k
        self.RR=RR
        self.cupy_device=cupy_device
        ...
        self.theta=cp.asarray(theta_points)
    ...
    def int_f(self,SS,batch_number,device,cupy_device):
        ...
        self.cupy_device=cupy_device
        cp.cuda.Device(self.cupy_device).use()
        w_sol=self.r**2*cp.sin(self.theta)*self.Phi*self.w
        del self.Phi
        if self.mea==1:
            if isinstance(SS, torch.Tensor):
                SS = SS.detach().cpu().numpy()
    ...
    def int_df_x1(self,SS,batch_number,device,cupy_device):
        ...
        cp.cuda.Device(self.cupy_device).use()
        result_y1=self.dPhi_x1*self.sub_y1
        w_sol_y1=self.r**2*cp.sin(self.theta)*result_y1*self.w
    ...
    def int_df_x2(self,SS,batch_number,device,cupy_device):
        ...
        cp.cuda.Device(self.cupy_device).use()
        result_y2=self.dPhi_x2*self.sub_y2
        w_sol_y2=self.r**2*cp.sin(self.theta)*result_y2*self.w
    ...
    def int_df_x3(self,SS,batch_number,device,cupy_device):
        ...
        cp.cuda.Device(self.cupy_device).use()
        result_y3=self.dPhi_x3*self.sub_y3
        w_sol_y3=self.r**2*cp.sin(self.theta)*result_y3*self.w
...
def boundary_data_circle(kk,number,p_b,ana_S,x_left,x_right,y_below,y_upper,z_min,z_max,center,r,eps,mea,batch_number_circle_mea,device,cupy_device):
    ...
    Args:
        ...
        eps (float64): Perturbation level.
    """
    g_circle=GaussLegendre3D_circle(number,center,r,p_b)
    ...
    R_mea_db_x3=[]
    for i in range(len(kk)):
        f_circle=forward_operator_circle(kk[i],RR_circle,w,r_points,theta_points,sub_y1,sub_y2,sub_y3,mea,device,cupy_device)
        u_mea_db_x1=(f_circle.int_df_x1(S_int_true,batch_number_circle_mea,device,cupy_device)).get()
        u_mea_db_x2=(f_circle.int_df_x2(S_int_true,batch_number_circle_mea,device,cupy_device)).get()
        u_mea_db_x3=(f_circle.int_df_x3(S_int_true,batch_number_circle_mea,device,cupy_device)).get()
        r1 = np.random.uniform(-1, 1, size=u_mea_db_x1.shape)
