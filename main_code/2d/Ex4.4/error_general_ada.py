...
def test(models1,models2,models3,M,Qx,Qy,w,af,Shape1,Shape2,temp,jump,x_min,x_max,y_min,y_max,x_left,x_right,y_below,y_upper,center_true,r_true,ana_S,grad_temp,device):
    ...
    for i in range(num_batches):
        ...
        if af=="mix":
            with torch.no_grad():
                basis_1=models1[0][0](batch_x,"Tanh").to(device)
                basis_2=models2[0][0](batch_x,"sigmoid").to(device)
                
            af_basis=torch.cat((basis_1,basis_2),dim=1)
            batch_output = torch.mm(af_basis,w)
        else:

            batch_output = torch.mm(models1[0][0](batch_x,af).reshape(-1,M).to(device),w)
