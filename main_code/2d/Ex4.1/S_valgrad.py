...
import numpy as np
import torch
def valgrad(models,af,Shape,M,all_points,w,ana_S,center,r,device):
    length=all_points.shape[0]
    all_g_p=torch.tensor(all_points,requires_grad=True).to(device)
    #S_combined = np.vstack(all_S_basis)
    w_last=torch.tensor(w.reshape(-1,1)).to(device)
    S_basis_=models[0][0](all_g_p,af,Shape).reshape(-1,M).to(device)
    S_num=torch.mm(S_basis_,w_last)
    grad_S_num = torch.autograd.grad(
        outputs=S_num,  # S_num_tensor 是模型的输出
        inputs=all_g_p,  # 对 X_test_tensor 求梯度
        grad_outputs=torch.ones_like(S_num),  # 梯度传播的权重
    ...
    print('S_l_inf=',S_epsilon.max()/S_o,'S_L_2=',math.sqrt(np.sum(S_epsilon**2)/length),'S_l_2=',math.sqrt(np.sum(S_epsilon**2)/length)/S_o) #为矩阵逐元素乘法
