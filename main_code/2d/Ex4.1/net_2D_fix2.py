...
def weights_init(m, R_m,b_min,b_max):
    if isinstance(m, nn.Linear):
        with torch.no_grad():
            # 对 hidden_layer_1 和 hidden_layer_2 进行特殊初始化
            if m.in_features == 1:
                # 权重初始化：每个神经元第一个权重为1，其余随机 [-R_m, R_m]
                m.weight[0, 0] = 1.0
...
class local_rep(nn.Module):
    def __init__(self, in_features, out_features, hidden_layers, M, x_min, x_max, y_min, y_max,r_min,r_max,b1_min,b1_max,b2_min,b2_max,width_min,width_max,height_min,height_max,peak_min,peak_max,v_min,v_max,K_min,K_max,R_m_for_init,af,Shape,device):
        ...
        self.y_min = y_min
        self.M = M
        self.a = torch.tensor([2.0/(x_max - x_min),2.0/(y_max - y_min)]).to(device) #每个单元半径的倒数
        self.x_0 = torch.tensor([(x_max + x_min)/2,(y_max + y_min)/2]).to(device) #每个单元的中心点
        self.hidden_layer= nn.Sequential(nn.Linear(in_features, self.hidden_features, bias=True)) #第一层
        ...
        self.af=af
        # 将它们注册为缓冲区 可以一起跟着隐藏层参数进行移动
        # self.register_buffer('a', self.a)
        # self.register_buffer('r_values', self.x_0)
        if self.af=="mix":
            ...
            weights_init(self.hidden_layer[0], R_m=R_m_for_init, b_min=-R_m_for_init, b_max=R_m_for_init) 
        elif self.af=="sigmoid" and Shape=="circle":
            weights_init(self.hidden_layer_1[0], R_m=R_m_for_init, b_min=b1_min, b_max=b1_max)
            weights_init(self.hidden_layer_2[0], R_m=R_m_for_init, b_min=b2_min, b_max=b2_max)
            self.r_values = torch.empty(self.M, dtype=torch.float64).uniform_(r_min, r_max).to(self.device)
            self.K = torch.empty(self.M, dtype=torch.float64).uniform_(K_min, K_max).to(self.device)
        elif self.af=="sigmoid" and Shape=="rec":
            weights_init(self.hidden_layer_1[0], R_m=R_m_for_init, b_min=b1_min, b_max=b1_max)
            weights_init(self.hidden_layer_2[0], R_m=R_m_for_init, b_min=b2_min, b_max=b2_max)
    ...
    def forward(self,x,af,Shape):
        y = self.a * (x - self.x_0) #标准化
        y = self.hidden_layer(y) #第一层 
        if af==0: #0:Tanh
            y=torch.tanh(y)
        elif af=="Tanh":
            y=torch.tanh(y) #y已经经过了一个hidden_layer了！
        elif af=="sigmoid" and Shape=="circle":
            y1 = self.hidden_layer_1(x[..., 0:1])
            y2 = self.hidden_layer_2(x[...,1:2])
            r_hat= torch.sqrt(y1**2+y2**2)#计算每个点到单元中心的距离   
            # 将这些随机数复制为 4 行
            r= self.r_values.repeat(y1.shape[0],1)
            K=self.K.repeat(y1.shape[0],1)
            y=(torch.sigmoid(K*(r-r_hat)))
            del r
        elif af=="sigmoid" and Shape=="rec":
            y1 = self.hidden_layer_1(x[..., 0:1])
            y2 = self.hidden_layer_2(x[..., 1:2])
            ...
            K=self.K.repeat(y1.shape[0],1)
            y=torch.sigmoid(-K*rect_distance)
        
        elif af=="Gauss":
            y1 = self.hidden_layer_1(x[..., 0:1])
            y2 = self.hidden_layer_2(x[...,1:2])
            r_hat_sq = y1**2 + y2**2 # Element-wise square, sum. Shape: [N, M]
...
def pre_define(Nx, Ny,M,R_m,af,tau_x_min,tau_x_max,tau_y_min,tau_y_max,r_min,r_max,b1_min,b1_max,b2_min,b2_max,width_min,width_max,height_min,height_max,peak_min,peak_max,v_min,v_max,K_min,K_max,Shape,device):  # Nx为x方向单元数 Ny为y方向单元数 M为基函数个数 Qx为单元x方向的单元 Qy为y方向配点个数
    models = []
    for k in range(Nx): #遍历x方向的单元数
        model_for_x = []
        #mea_points=[]
        x_min = (tau_x_max-tau_x_min)/Nx * k+tau_x_min #L/Nx为单元直径 
        ...
        for n in range(Ny):
            ...
            model = local_rep(in_features = 2, out_features = 2, hidden_layers = 1, M = M, x_min = x_min, 
                              x_max = x_max, y_min = y_min, y_max = y_max,r_min=r_min,r_max=r_max,b1_min=b1_min,b1_max=b1_max,b2_min=b2_min,b2_max=b2_max,width_min=width_min,width_max=width_max,height_min=height_min,height_max=height_max,peak_min=peak_min,peak_max=peak_max,v_min=v_min,v_max=v_max,K_min=K_min,K_max=K_max,R_m_for_init=R_m,af=af,Shape=Shape,device=device).to(device)
            #model.apply(lambda m: weights_init(m, R_m=R_m,b_below=b_low,b_upper=b_upper))  #初始化权值和偏置元
