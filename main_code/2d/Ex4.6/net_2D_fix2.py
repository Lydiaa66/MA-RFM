...
class local_rep(nn.Module):
    def __init__(self, in_features, out_features, hidden_layers, M,M_noise,b1_noise,b2_noise, x_min, x_max, y_min, y_max,r_min,r_max,b1_min,b1_max,b2_min,b2_max,width_min,width_max,height_min,height_max,peak_min,peak_max,v_min,v_max,K_min,K_max,R_m_for_init,af,Shape,device):
        ...
        if self.af=="mix":
            ...
            self.K = torch.empty(self.M, dtype=torch.float64).uniform_(K_min, K_max).to(self.device)
        elif self.af=="sigmoid" and Shape=="general":
            self.K = torch.empty(self.M, dtype=torch.float64).uniform_(K_min, K_max).to(self.device)
        elif self.af=="sigmoid" and Shape=="noise":
            self.hidden_layer_1 = nn.Sequential(nn.Linear(1, M_noise, bias=True))
            self.hidden_layer_2 = nn.Sequential(nn.Linear(1, M_noise, bias=True))
            weights_init(self.hidden_layer_1[0], R_m=R_m_for_init, b_min=b1_min, b_max=b1_max)
            weights_init(self.hidden_layer_2[0], R_m=R_m_for_init, b_min=b2_min, b_max=b2_max)
            self.hidden_layer_1[0].bias.data = torch.tensor(b1_noise, dtype=torch.float64).to(device)
            self.hidden_layer_2[0].bias.data = torch.tensor(b2_noise, dtype=torch.float64).to(device)
            self.r_values = torch.empty(self.M_noise, dtype=torch.float64).uniform_(r_min, r_max).to(self.device)
            self.K = torch.empty(self.M_noise, dtype=torch.float64).uniform_(K_min, K_max).to(self.device)
        elif self.af=="Gauss" or self.af=="continue_gauss":
            weights_init(self.hidden_layer_1[0], R_m=R_m_for_init, b_min=b1_min, b_max=b1_max)
            weights_init(self.hidden_layer_2[0], R_m=R_m_for_init, b_min=b2_min, b_max=b2_max)
            self.r_values = torch.empty(self.M, dtype=torch.float64).uniform_(r_min, r_max).to(self.device)
            self.K = torch.empty(self.M, dtype=torch.float64).uniform_(K_min, K_max).to(self.device)
    ...
    def forward(self,x,af,Shape,signed_distances):
        y = self.a * (x - self.x_0)
        ...
        elif af=="sigmoid" and Shape=="general":
            K=self.K.repeat(y.shape[0],1)
            y=torch.sigmoid(K*signed_distances)
            
        elif af=="sigmoid" and Shape=="noise":
            K=self.K.repeat(y.shape[0],1)
            y1 = self.hidden_layer_1(x[..., 0:1])
            y2 = self.hidden_layer_2(x[...,1:2])
            r_hat= torch.sqrt(y1**2+y2**2)
