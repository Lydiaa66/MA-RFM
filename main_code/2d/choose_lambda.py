...
def Inconsistency(all_mat,eta_value,nu,w):
    ...
    U_null_space_basis = U[:, r:] # Left null space.
    # Generate a random vector in the left null space.
    np.random.seed(2)
    random_coeffs = np.random.randn(U.shape[0]- r, 1).reshape(-1,1)
    eta_vec = U_null_space_basis.get() @ random_coeffs  # Generate an arbitrary vector in the orthogonal complement.

    # Scale its norm to the desired value.
