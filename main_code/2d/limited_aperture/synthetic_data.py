...
def dir_neu(N_m,N_m_inv,alpha,kk,R,rho,n_trun,F_mea_b,temp):
    F_mea_b_complex = F_mea_b[..., 0] + 1j * F_mea_b[..., 1]

    theta = np.linspace(0, 2 * np.pi*alpha, N_m, endpoint=temp)
    ...
    if alpha==1:
        ...
        U_fourier = (alpha*2*np.pi/ (N_m))*(1/(2*np.pi)) * np.sum(F_mea_b_complex[:, np.newaxis, :] * exp_term[np.newaxis, :, :], axis=2)
