...
def test(models0,model_relu1,model_relu2,model_gauss1,model_gauss2,sample_s,w,af,temp,jump,x_min,x_max,y_min,y_max,z_min,z_max,x_left,x_right,y_below,y_upper,center_true,r_true,ana_S,grad_temp,ratio1,ratio2,device):
    ...
    for i in range(num_batches):
        ...
        if af=="mixer":
            ...
            batch_output = torch.mm(models0(batch_x,af,device).to(device),w)
            if grad_temp==1:
                grad_S_batch = torch.autograd.grad(
                    outputs=batch_output,
                    inputs=batch_x,
    ...
    S_true=ana_S(sample_s,[],[],[],[],[],[],center_true[0],center_true[1],r_true).reshape(test_Qx+1,test_Qy+1,test_Qz+1)
    ...
    amp_x1=np.max(np.abs(S_num_1D_x1))
    peak_index_x1 = np.argmax(S_true_1D_x1)
    half_max_x1 =amp_x1/2
    half_max_indices_x1= np.where((S_num_1D_x1 >= half_max_x1 - 10**(-2.2)) & (S_num_1D_x1 <= half_max_x1 + 10**(-2.2)))[0]
    x_devide = np.linspace(x_min, x_max, test_Qx + 1)
    x_at_half_max=x_devide[half_max_indices_x1]
    ...
    S_num_1D_x2=S_num[int(ratio2[0]*S_num.shape[0])+1-1,:,int(ratio2[2]*S_num.shape[2])+1-1]
    S_true_1D_x2=S_true[int(ratio2[0]*S_true.shape[0])+1-1,:,int(ratio2[2]*S_true.shape[2])+1-1]
    amp_x2=np.min(S_num_1D_x2)
    peak_index_x2 = np.argmax(S_true_1D_x2)
    half_max_x2 =amp_x2/2
    half_max_indices_x2= np.where((S_num_1D_x2 >= half_max_x2 - 10**(-2.2)) & (S_num_1D_x2 <= half_max_x2 + 10**(-2.2)))[0]
    ...
    if temp:            
        ...
        min_val = np.min(S_num_x1_mid)
        max_val = np.max(S_num_x1_mid)
        min_idx = np.unravel_index(np.argmin(S_num_x1_mid), S_num_x1_mid.shape)
        max_idx = np.unravel_index(np.argmax(S_num_x1_mid), S_num_x1_mid.shape)

        y_vals = np.linspace(y_min, y_max, S_num_x1_mid.shape[0])
        z_vals = np.linspace(z_min, z_max, S_num_x1_mid.shape[1])
        y_coord_min, z_coord_min = y_vals[min_idx[0]], z_vals[min_idx[1]]
        y_coord_max, z_coord_max = y_vals[max_idx[0]], z_vals[max_idx[1]]

        heat_map_2D.improved_plot(
            S_num_x1_mid,
            title="Num_S(0.5, x2, x3)",
        ...
        axs[0, 0].text(max_idx[0], max_idx[1], f"max={max_val:.2f},[{y_coord_max:.2f}, {z_coord_max:.2f}]", color="white", fontsize=8,
                    ha='center', va='bottom', bbox=dict(facecolor='black', alpha=0.6, boxstyle='round,pad=0.2'))
        axs[0, 0].text(min_idx[0], min_idx[1], f"min={min_val:.2f},[{y_coord_min:.2f}, {z_coord_min:.2f}]", color="white", fontsize=8,
                    ha='center', va='top', bbox=dict(facecolor='black', alpha=0.6, boxstyle='round,pad=0.2'))

        heat_map_2D.improved_plot(
            S_true_x1_mid,
            title="Exact_S(0.5, x2, x3)",
            x_min=y_min, x_max=y_max,
            y_min=z_min, y_max=z_max,
            xlabel="x2", ylabel="x3",
            cmap="rainbow",shrink=1,
        ...
        min_val = np.min(S_num_x2_mid)
        max_val = np.max(S_num_x2_mid)
        min_idx = np.unravel_index(np.argmin(S_num_x2_mid), S_num_x2_mid.shape)
        max_idx = np.unravel_index(np.argmax(S_num_x2_mid), S_num_x2_mid.shape)

        y_vals = np.linspace(y_min, y_max, S_num_x2_mid.shape[0])
        ...
        axs[1, 0].text(max_idx[0], max_idx[1], f"max={max_val:.2f},[{y_coord_max:.2f}, {z_coord_max:.2f}]", color="white", fontsize=8,
                    ha='center', va='bottom', bbox=dict(facecolor='black', alpha=0.6, boxstyle='round,pad=0.2'))
        axs[1, 0].text(min_idx[0], min_idx[1], f"min={min_val:.2f},[{y_coord_min:.2f}, {z_coord_min:.2f}]", color="white", fontsize=8,
                    ha='center', va='top', bbox=dict(facecolor='black', alpha=0.6, boxstyle='round,pad=0.2'))

        heat_map_2D.improved_plot(
            S_true_x2_mid,
            title="Exact_S(x1, 0.5, x3)",
            x_min=x_min, x_max=x_max,
            y_min=z_min, y_max=z_max,
            xlabel="x1", ylabel="x3",
            cmap="rainbow",shrink=1,
    ...
    if jump:
          ...
          for i in range(len(half_max_indices_x1)):
            idx=half_max_indices_x1[i]
            plt.axvline(x=half_max_indices_x1[i], color='g', linestyle='--')
            x_val = x_devide[idx]
          ...
          for i in range(len(half_max_indices_x2)):
            idx=half_max_indices_x2[i]
            plt.axvline(x=idx, color='g', linestyle='--')
            x_val = x_devide[idx]
            plt.text(idx+1, S_num_1D_x2[idx], f"x={x_val:.2f}", color='green', fontsize=9, rotation=0, va='bottom')
