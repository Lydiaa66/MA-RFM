...
def detect_shape(grad, X, Y, tau_x_min, tau_x_max, tau_y_min, tau_y_max,para_abs ,para_grad, eps,mini_samples,
    ...
    for i, cluster_info in enumerate(segmented_results):
        ...
        is_donut = False
        if aspect_ratio < 1.2 and min_dist_to_center > 0.2 * r_mean and min_dist_to_center < 0.5 * r_mean:
             is_donut = True        
        if is_donut:
            shape_type = "Donut"
        ...
        sc = ax.scatter(cluster_pts[:, 0], cluster_pts[:, 1], c=s_vals, cmap="jet", 
    ...
    ax.set_xticklabels(x_tick_labels, rotation=0, fontsize=20)
