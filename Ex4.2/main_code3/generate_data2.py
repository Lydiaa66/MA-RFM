...
def boundary_data_rec(kk,number,p_b,ana_S,x_left,x_right,y_below,y_upper,center,r,eps,mea,batch_number_rec_mea,device,cupy_device):
    ...
    for i in range(len(kk)):
        f_rec=forward_operator_rec(kk[i],RR_rec,w,diff,normal,mea,device,cupy_device)
        u_mea_db=(f_rec.int_df(S_int_true,batch_number_rec_mea)).get()
        r = np.random.uniform(-1, 1, size=u_mea_db.shape)
        abs_du=np.abs(u_mea_db)
        u_per_db=u_mea_db+eps*r*abs_du*np.exp(1j * np.pi * r)
...
class forward_operator_circle():
    def __init__(self,k,RR,w,r_points,sub_y1,sub_y2,mea,device,cupy_device):
        self.mea=mea
        self.k=k
        self.RR=RR
