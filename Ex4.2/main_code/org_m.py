...
def A(b_matrix,db_x1_matrix):
    ...
    mat=np.concatenate((A_b,B_b,A_db_x1,B_db_x1),axis=0)

    return mat

def F(F_mea_b,F_mea_db_x1):
    F_mea_b=F_mea_b.reshape(-1,F_mea_b.shape[-1])
    F_mea_db_x1=F_mea_db_x1.reshape(-1,F_mea_db_x1.shape[-1])
    F=np.concatenate((F_mea_b[:,0:1],F_mea_b[:,1:],F_mea_db_x1[:,0:1],F_mea_db_x1[:,1:]),axis=0)
    return F
