...
import heat_map_2D
import generate_data2,heat_map_2D,smooth,net_2D_fix2
np.random.seed(2)
...
def test(models0,models1,models2,models3,sample_s,w,af,Shape1,Shape2,sign_distances,temp,jump,x_min,x_max,y_min,y_max,x_left,x_right,y_below,y_upper,center_true,r_true,ana_S,pos,grad_temp,device):
    ...
    for i in range(num_batches):
        ...
        if af=="mix+gauss":
            if isinstance(sign_distances,np.ndarray):
                sign_distances_loc=torch.tensor(sign_distances[start_idx:end_idx,:]).to(device) 
            basis_0=models0(batch_x,"Tanh",[],[]).to(device)
            basis_1=models1(batch_x,"sigmoid",Shape1,sign_distances_loc).to(device)
            basis_2=models2(batch_x,"sigmoid",Shape2,[]).to(device)
            basis_3=models3(batch_x,"continue_gauss",[],[]).to(device)
        ...
        if af=="mix":
            ...
            with torch.no_grad():
                basis_1=models1(batch_x,"sigmoid",Shape1,[]).to(device)
                basis_2=models2(batch_x,"sigmoid",Shape2,[]).to(device)
            af_basis=torch.cat((basis_1,basis_2),dim=1)
            batch_output = torch.mm(af_basis,w)
        elif af=="sigmoid" and Shape1=="general" and Shape2 !="noise":
            basis = models0(batch_x, "sigmoid",Shape1,sign_distances[start_idx:end_idx,:])
            batch_output = torch.mm(basis, w)
            del basis
        elif af=="sigmoid" and Shape1=="general" and Shape2=="noise": 
            basis_0 = models0(batch_x, "sigmoid",Shape1,sign_distances[start_idx:end_idx,:])
            basis_1 = models1(batch_x, "sigmoid",Shape2,[])
            basis = torch.cat((basis_0,basis_1), dim=1)
            batch_output = torch.mm(basis, w)
            del basis
        else:
            batch_output = torch.mm(models0(batch_x,af,Shape1,[]).to(device),w)

        S_num_pred[start_idx:end_idx, :] = batch_output
        del batch_output
    del sign_distances
    
    if af=="mix" and grad_temp==1:
