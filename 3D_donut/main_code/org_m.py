...
def A(b_matrix,db_x1_matrix,db_x2_matrix,db_x3_matrix,condition):
    if condition=="Cauchy":
...
def F(F_mea_b,F_mea_db_x1,F_mea_db_x2,F_mea_db_x3,condition):
    if condition=="Cauchy":
...
def L_curve(mat,F,lamb_regu,cupy_device):
    iter=len(lamb_regu)
    res=np.zeros([1,iter])
