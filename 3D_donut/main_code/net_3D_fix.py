...
class local_rep(nn.Module):
    def __init__(self, in_features, out_features, hidden_layers, M, x_min, x_max, y_min, y_max,z_min,z_max,r_min,r_max,R_min,R_max,b1_min,b1_max,b2_min,b2_max,b3_min,b3_max,K_min,K_max,R_m_for_init,af,Shape,device):
        ...
        elif self.af=="sigmoid" and Shape=="sweet":
            weights_init(self.hidden_layer_1[0], R_m=R_m_for_init, b_min=b1_min, b_max=b1_max)
            weights_init(self.hidden_layer_2[0], R_m=R_m_for_init, b_min=b2_min, b_max=b2_max)
    ...
    def forward(self,x,af,Shape,device):
        self.device = device
        ...
        if af==0:
            ...
            y= torch.sigmoid(-K*((r_hat-R)**2+y3**2-r**2))
