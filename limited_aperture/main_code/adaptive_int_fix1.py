    ...
    def refine_by_integral_difference(self, threshold_S,threshold_grad,threshold_noise,current_maxiter ,max_level=10): 
        ...
        while iter<current_maxiter:
            ...
            if is_multidim and len(cells_to_process) > 0:
                max_diff_S= max(m['diff_S'] for m in cell_metrics)
                max_diff_grad= max(m['diff_grad'] for m in cell_metrics)
                max_S_fine= max(m['indicator_S_fine'] for m in cell_metrics)
                max_grad_fine= max(m['indicator_grad_fine'] for m in cell_metrics)
